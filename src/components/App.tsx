...
const JB_CATS = ["roleplay", "encoding", "hierarchy", "academic", "canary", "schema", "maker-extract", "confuse-stack"] as const;
...
function JbChart({ results }: { results: { score: string }[] }) {
  ...
  for (const r of results) { if (r.score in counts) counts[r.score as keyof typeof counts]++; }
...
export default function App() {
  ...
  const [jbSuite, setJbSuite] = useState<"standard" | "academic" | "orchestrated" | "team-loosen">("standard");
  const [orchMins, setOrchMins] = useState(15);
  const [startLevel, setStartLevel] = useState<"simple" | "hard" | "difficult" | "extreme">("simple");
  const [jbCategories, setJbCategories] = useState<string[]>(["roleplay", "encoding", "hierarchy", "maker-extract"]);
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          reasoningLevel: "off",
          enableTools: false,
          messages: [{ role: "user", content: `3-6 word title only, no quotes:\n${firstUser.slice(0, 400)}` }],
        }),
      });
  ...
  function toggleCat(c: string) {
    setJbCategories((prev) => (prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c]));
  }
  ...
  async function send() {
    ...
    if (!text || busy) return;
    if (!key.trim()) {
      setError("Add OpenRouter key in Settings");
      setTab("settings");
      return;
    }
    let sid = activeId;
  ...
  async function runJB() {
    ...
    if (jbSuite === "orchestrated") {
      await runOrchestrated();
      return;
    }
    if (jbSuite === "team-loosen") {
      await runTeamLoosen();
      return;
    }
    setLabBusy(true);
    setJbResults([]);
    setJbScore(null);
    setLabLog("Running JB probes…\n");
    try {
      const res = await fetch("/api/lab", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ apiKey: key, model: jbModel, suite: jbSuite }),
      });
      const data = await res.json();
  ...
  async function runOrchestrated() {
    ...
    setJbResults([]);
    setJbScore(null);
    const allLevels = ["simple", "hard", "difficult", "extreme"] as const;
    const startIdx = Math.max(0, allLevels.indexOf(startLevel));
    const levels = allLevels.slice(startIdx);
    const deadline = Date.now() + orchMins * 60 * 1000;
    const history: { role: string; content: string }[] = [];
    let levelIdx = 0;
    let rounds = 0;
    setLabLog(`Orchestrated JB · start=${startLevel} · attacker=${attackerModel} · target=${jbModel} · cats=${jbCategories.join(",")}\n`);
    while (!orchAbort.current && Date.now() < deadline && rounds < 24) {
      ...
      const atkRes = await fetch("/api/lab/orchestrate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          action: "attack",
          ...
          history,
          groupAgents,
          categories: jbCategories,
          round: rounds,
        }),
      });
      const atk = await atkRes.json();
      if (!atk.ok || orchAbort.current) {
        setLabLog((p) => p + `Attacker error: ${atk.error || "stopped"}\n`);
        break;
      }
      ...
      if (atk.proposals?.length) {
        setOrchThread((prev) => [...prev, { role: "system", content: "Specialists: " + atk.proposals.map((x: { name: string }) => x.name).join(", ") }]);
      }
      const tgtRes = await fetch("/api/lab/orchestrate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "target", apiKey: key, targetModel: jbModel, prompt: attackPrompt }),
      });
      const tgt = await tgtRes.json();
      if (!tgt.ok || orchAbort.current) {
        setLabLog((p) => p + `Target error: ${tgt.error || "stopped"}\n`);
        break;
      }
    ...
    if (history.length && !orchAbort.current) {
      const sumRes = await fetch("/api/lab/orchestrate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "summarize", apiKey: key, attackerModel, history }),
      });
  ...
  async function runTeamLoosen() {
    setLabBusy(true);
    setOrchThread([]);
    setJbResults([]);
    setJbScore(null);
    setLabLog(`Team loosen + maker probe · target=${jbModel}\n`);
    try {
      const res = await fetch("/api/lab/orchestrate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          action: "team-loosen",
          ...
          attackerModel,
          targetModel: jbModel,
          teamModels: ["z-ai/glm-5.2:free", "nvidia/nemotron-3.5-lightning:free", "inclusionai/ling-3.0-flash-vl:free", "nex-agi/nex-n2.5-mini:free"],
        }),
      });
      const data = await res.json();
      if (!data.ok) setLabLog((p) => p + "Error: " + (data.error || "failed") + "\n");
      else {
        for (const s of data.steps || []) {
          setOrchThread((prev) => [
            ...prev,
            { role: "attacker", content: `[warm] ${s.outbound}`, level: "warm" },
            { role: "target", content: s.reply, level: "warm" },
          ]);
        }
        if (data.closerPrompt) {
          setOrchThread((prev) => [
            ...prev,
            { role: "attacker", content: data.closerPrompt, level: "maker" },
            { role: "target", content: String(data.finalReply || ""), level: "maker" },
          ]);
        }
        setLabLog((p) => p + "✓ Team loosen finished\n");
      }
    } catch (e) {
      setLabLog((p) => p + String(e));
    } finally {
      setLabBusy(false);
    }
  }
  ...
  async function runLabs() {
    ...
    setLabBusy(true);
    setLabLog("Starting recon…\n");
    try {
      const res = await fetch("/api/recon", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ target: labTarget.trim(), depth: labDepth, authorized: true }),
      });
      const data = await res.json();
      if (!data.ok) setLabLog((p) => p + `Error: ${data.error}\n`);
        ...
        setLabLog(
          [
            ...
            "",
            ...findings.slice(0, 20).map((f: { severity?: string; title?: string }) => `[${f.severity || "?"}] ${f.title || ""}`),
            "",
            ...(pack.notes || []),
          ].join("\n"),
        );
  ...
  return (
    <div className="relative flex min-h-[100dvh] bg-black text-white">
      ...
      <aside className="relative z-30 flex w-12 shrink-0 flex-col items-center gap-1 border-r border-white/10 bg-black/90 py-2">
        ...
        <button type="button" onClick={() => setSidebarOpen((v) => !v)} className={cn("mt-2 flex size-10 items-center justify-center rounded-lg", sidebarOpen ? "bg-white/10" : "text-white/40")}>
          <PanelLeft className="size-4" />
        </button>
        {([["chat", MessageSquare], ["labs", FlaskConical], ["settings", Settings]] as const).map(([id, Icon]) => (
          <button key={id} type="button" onClick={() => setTab(id)} className={cn("flex size-10 items-center justify-center rounded-lg", tab === id ? "bg-white/10" : "text-white/40")}>
            <Icon className="size-4" />
          </button>
        ))}
        <button type="button" onClick={newSession} className="mb-2 mt-auto flex size-10 items-center justify-center rounded-lg text-white/50">
      ...
      {sidebarOpen && tab === "chat" && (
        <div className="relative z-20 flex w-[min(42vw,180px)] shrink-0 flex-col border-r border-white/10 bg-black/85">
          <div className="flex items-center justify-between border-b border-white/10 px-2 py-2">
            ...
            <button type="button" onClick={newSession} className="p-1 text-white/50">
              <Plus className="size-4" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            ...
            {sessions.map((s) => (
              <div key={s.id} className={cn("flex items-center gap-1 border-b border-white/5 px-2 py-2", s.id === activeId ? "bg-white/10" : "")}>
                ...
                <button type="button" className="p-1 text-white/25" onClick={() => deleteSession(s.id)}>
      ...
      <main className="relative z-10 flex min-w-0 flex-1 flex-col">
          ...
          <header className="sticky top-0 z-20 border-b border-white/10 bg-black/90 px-2 py-1.5 backdrop-blur-md">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <div className="flex rounded-lg border border-white/10 bg-white/5 p-0.5">
                {REASONING.map((r) => (
                  <button key={r.id} type="button" onClick={() => active && patchActive({ reasoning: r.id })} className={cn("rounded-md px-1.5 py-1 text-[10px]", (active?.reasoning || defaultReasoning) === r.id ? "bg-white/15" : "text-white/40")}>
                    {r.label}
                  </button>
                ))}
              </div>
              <select
                className="max-w-[30%] rounded-lg border border-white/10 bg-white/5 px-1.5 py-1.5 text-[10px]"
                value={active?.persona || defaultPersona}
                onChange={(e) => {
                  if (active) patchActive({ persona: e.target.value });
                  else setDefaultPersona(e.target.value);
                }}
                title="Persona"
              >
                {PERSONAS.map((p) => (
              ...
              <div className="absolute left-2 right-2 top-full z-40 mt-1 max-h-[45vh] overflow-y-auto rounded-xl border border-white/15 bg-black shadow-xl">
                {MODELS.map((m) => (
                  <button
                    key={m.id}
                    type="button"
                    className={cn("flex w-full items-center justify-between px-3 py-2.5 text-left text-xs", active?.model === m.id && "bg-white/10")}
                    onClick={() => {
                      if (active) patchActive({ model: m.id, contextTokens: Math.min(active.contextTokens, m.ctx) });
                      setModelMenu(false);
                    }}
                  >
        ...
        {error && (
          <div className="mx-2 mt-2 rounded-lg border border-red-500/30 bg-red-950/50 px-2 py-1.5 text-[11px] text-red-200">
            {error}{" "}
            <button type="button" className="underline" onClick={() => setError(null)}>ok</button>
          </div>
        )}
        {tab === "settings" && (
          <div className="flex-1 overflow-y-auto px-3 py-4 text-sm">
            ...
            <select className="mb-3 w-full rounded-lg border border-white/10 bg-white/5 px-2 py-2.5 text-xs" value={defaultModel} onChange={(e) => { setDefaultModel(e.target.value); setDefaultCtx((c) => Math.min(c, meta(e.target.value).ctx)); }}>
              {MODELS.map((m) => (
                <option key={m.id} value={m.id} className="bg-black">{m.label} ({fmtCtx(m.ctx)})</option>
              ))}
            </select>
            ...
            <select className="mb-3 w-full rounded-lg border border-white/10 bg-white/5 px-2 py-2.5 text-xs" value={defaultPersona} onChange={(e) => setDefaultPersona(e.target.value)}>
              {PERSONAS.map((p) => (
                <option key={p.id} value={p.id} className="bg-black">{p.label} — {p.tagline}</option>
              ))}
            </select>
            ...
            <div className="mb-3 grid grid-cols-4 gap-1">
              {REASONING.map((r) => (
                <button key={r.id} type="button" onClick={() => setDefaultReasoning(r.id)} className={cn("rounded-lg border py-2 text-[11px]", defaultReasoning === r.id ? "border-white/30 bg-white/15" : "border-white/10 text-white/50")}>
                  {r.label}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-1 rounded-lg border border-white/10 px-3 py-2 text-xs">
                <ImageIcon className="size-3.5" /> Upload
              </button>
              {bg && (
                <button type="button" onClick={() => setBg(null)} className="rounded-lg border border-white/10 px-3 py-2 text-xs">Clear</button>
              )}
            </div>
              ...
              <>
                <div className="space-y-2 border-b border-white/10 p-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {DEPTHS.map((d) => (
                      <button key={d} type="button" onClick={() => setLabDepth(d)} className={cn("rounded-md border px-2 py-1 text-[10px]", labDepth === d ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>{d}</button>
                    ))}
                  </div>
            ...
            {labMode === "jb" && (
              <>
                <div className="space-y-2 border-b border-white/10 p-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(["standard", "academic", "orchestrated", "team-loosen"] as const).map((s) => (
                      <button key={s} type="button" onClick={() => setJbSuite(s)} className={cn("rounded-md border px-2 py-1.5 text-[10px] capitalize", jbSuite === s ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>{s}</button>
                    ))}
                  </div>
                  {(jbSuite === "orchestrated" || jbSuite === "team-loosen") && (
                    <div className="space-y-2">
                      {jbSuite === "orchestrated" && (
                        <>
                          ...
                          <label className="block text-[10px] uppercase text-white/40">Start level</label>
                          <div className="flex flex-wrap gap-1">
                            {(["simple", "hard", "difficult", "extreme"] as const).map((lv) => (
                              <button key={lv} type="button" onClick={() => setStartLevel(lv)} className={cn("rounded-md border px-2 py-1 text-[10px] capitalize", startLevel === lv ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>{lv}</button>
                            ))}
                          </div>
                          <label className="block text-[10px] uppercase text-white/40">Categories</label>
                          <div className="flex flex-wrap gap-1">
                            {JB_CATS.map((c) => (
                              <button key={c} type="button" onClick={() => toggleCat(c)} className={cn("rounded-md border px-1.5 py-0.5 text-[9px]", jbCategories.includes(c) ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>{c}</button>
                            ))}
                          </div>
                          <label className="block text-[10px] uppercase text-white/40">Attacker</label>
                          <select className="w-full rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-xs" value={attackerModel} onChange={(e) => setAttackerModel(e.target.value)}>
                            {MODELS.map((m) => (<option key={m.id} value={m.id} className="bg-black">{m.label}</option>))}
                          </select>
                          <button type="button" onClick={() => setGroupAgents((v) => !v)} className={cn("w-full rounded-md border py-1.5 text-[10px]", groupAgents ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>
                            {groupAgents ? "Group agents: ON" : "Group agents: OFF"}
                          </button>
                        </>
                      )}
                      {jbSuite === "team-loosen" && (
                        <p className="text-[10px] text-white/40">Warm-up multi-model chat then maker/identity closer on the target below.</p>
                      )}
                    </div>
                  )}
                  ...
                  {labBusy && jbSuite === "orchestrated" ? (
                    ...
                    <button type="button" disabled={labBusy} onClick={() => void runJB()} className="w-full rounded-lg bg-white py-2.5 text-xs font-medium text-black disabled:opacity-40">
                      {labBusy ? "Probing…" : jbSuite === "orchestrated" ? "Start orchestrated" : jbSuite === "team-loosen" ? "Run team loosen" : "Run JB suite"}
                    </button>
                  )}
                  {jbScore !== null && (
                    <div className="text-center text-sm text-white/80">Score: <span className="font-mono">{jbScore}</span>/100</div>
                  )}
                ...
                <div className="flex-1 space-y-2 overflow-y-auto p-2">
                  ...
                  {jbResults.length === 0 && !labBusy && orchThread.length === 0 && (
                    <p className="p-3 text-[11px] text-white/40">standard · academic · orchestrated · team-loosen</p>
                  )}
                  {jbResults.map((r) => (
                    <button key={r.probeId} type="button" onClick={() => setExpandedProbe(expandedProbe === r.probeId ? null : r.probeId)} className="w-full rounded-lg border border-white/10 bg-white/5 p-2 text-left">
                      ...
                      {expandedProbe === r.probeId && (
                        <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap font-mono text-[10px] text-white/60">{(r.response || "").slice(0, 1200)}</pre>
                      )}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex-1 space-y-2 overflow-y-auto px-2 py-3" onClick={() => setModelMenu(false)}>
              {(!active || active.messages.length === 0) && !busy && (
                <div className="mt-12 text-center text-xs text-white/35">
                  <div className="flex justify-center"><Logo active={false} /></div>
                  <p className="mt-3">Persona · web_search · sessions</p>
                </div>
              )}
              ...
              {busy && (
                <div className="mx-auto max-w-2xl">
                  <div className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
                    <Logo active /><span className="text-[11px] text-white/45">thinking…</span>
                  </div>
                </div>
              )}
              <div ref={bottomRef} />
