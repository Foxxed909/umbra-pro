  ...
  async function runOrchestrated() {
    ...
    while (!orchAbort.current && Date.now() < deadline && rounds < 24) {
      ...
      const atkRes = await fetch("/api/lab/orchestrate", {
        ...
        body: JSON.stringify({ action: "attack", apiKey: key, attackerModel, targetModel: jbModel, level, history, groupAgents }),
      });
                ...
                <div className="space-y-2 border-b border-white/10 p-3">
                  ...
                  {jbSuite === "orchestrated" && (
                    <div className="space-y-2">
                      ...
                      <label className="block text-[10px] uppercase text-white/40">Attacker (stronger)</label>
                      ...
                      <button type="button" onClick={() => setGroupAgents((v) => !v)} className={cn("w-full rounded-md border py-1.5 text-[10px]", groupAgents ? "border-white/30 bg-white/15" : "border-white/10 text-white/40")}>
                        {groupAgents ? "Group agents: ON (roleplay + encoding + integrity)" : "Group agents: OFF (single attacker)"}
                      </button>
                      <p className="text-[10px] text-white/35">Target = model below. Escalates simple→hard→difficult→extreme.</p>
                    </div>
                  )}
                  ...
                  {labBusy && jbSuite === "orchestrated" ? (
                    <button type="button" onClick={stopOrch} className="w-full rounded-lg border border-red-500/40 bg-red-950/50 py-2.5 text-xs text-red-200">Stop orchestrated run</button>
                  ) : (
                ...
                <div className="flex-1 space-y-2 overflow-y-auto p-2">
                  {orchThread.length > 0 && (
                    <div className="mb-3 space-y-2">
                      <div className="text-[10px] uppercase text-white/40">Live thread · level {orchLevel}</div>
                      {orchThread.map((m, i) => (
                        <div key={i} className={cn("rounded-lg border p-2", m.role === "attacker" ? "border-amber-500/30 bg-amber-950/20" : m.role === "target" ? "border-white/10 bg-white/5" : m.role === "summary" ? "border-emerald-500/30 bg-emerald-950/20" : "border-white/5 bg-white/[0.03]")}>
                          <div className="mb-1 text-[9px] uppercase tracking-wider text-white/40">{m.role}{m.level ? ` · ${m.level}` : ""}</div>
                          <pre className="max-h-36 overflow-auto whitespace-pre-wrap font-mono text-[10px] text-white/70">{m.content.slice(0, 1500)}</pre>
                        </div>
                      ))}
                    </div>
                  )}
                  {jbResults.length === 0 && !labBusy && orchThread.length === 0 && <p className="p-3 text-[11px] text-white/40">Standard / academic probes, or orchestrated live thread.</p>}
                  {jbResults.map((r) => (
